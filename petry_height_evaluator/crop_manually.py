...
class ImageCropper:
    ...
    def save_crop(self):
        if None not in (self.start_x, self.start_y, self.end_x, self.end_y):
            ...
            # Define the folder path for cropped images
            cropped_folder = os.path.join(self.image_folder, 'cropped')
            # Create the 'cropped' folder if it doesn't exist
            if not os.path.exists(cropped_folder):
                os.makedirs(cropped_folder)
            cropped_image.save(os.path.join(cropped_folder,f"cropped_{self.images[self.current_image_index]}"))
